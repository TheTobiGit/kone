...
describe("bot marks", () => {
  test("a mark carries no document references", () => {
    ...
    for (const shape of BOT_SHAPES) {
      for (const expression of BOT_EXPRESSIONS) {
        const svg = botMark({ ...DEFAULT_BOT, shape: shape.id, expression: expression.id });
  ...
  test("a shape's mark differs from the one beside it", () => {
    const marks = new Set(BOT_SHAPES.map((s) => botMark({ ...DEFAULT_BOT, shape: s.id })));
    expect(marks.size).toBe(BOT_SHAPES.length);
  });
...
describe("a ground to read against", () => {
  ...
  test("the ground follows the colour, not the shape or the mood", () => {
    const a: AgentBot = { shape: "circle", color: "teal", expression: "neutral" };
    const b: AgentBot = { shape: "droplet", color: "teal", expression: "sleepy" };
...
describe("reading one back", () => {
  ...
  test("a bot naming what this build no longer ships falls back per field", () => {
    expect(readBot({ shape: "sprocket", color: "teal", expression: "wary" })).toEqual({
      shape: DEFAULT_BOT.shape,
      color: "teal",
      expression: "wary",
    });
  });
  ...
  test("a summary names all three choices", () => {
    const line = botSummary({ shape: "pebble", color: "teal", expression: "curious" });
