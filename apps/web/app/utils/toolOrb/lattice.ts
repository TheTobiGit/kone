// The sphere-lattice modes: globe (searching) and rubik (solving). Both
// draw a lat/long dot field with mode-specific motion, then hand off to
...
export const drawGlobe: ModeDraw = (ctx, size, t, dark, hueDeg, o) => {
  const spin = 0.5;
  const cx = size / 2;
  const cy = size / 2;
...
export const drawRubik: ModeDraw = (ctx, size, t, dark, hueDeg, o) => {
  const cx = size / 2;
  const cy = size / 2;
  const R = (size / 2) * 0.82;
