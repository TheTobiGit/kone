...
// tumble (spin 0), leaving the traveling undulation on a fixed band.
//
// The same painter drives kone's "thinking" (ring), via the `faceOn` flag:
...
export const drawRibbon: ModeDraw = (ctx, size, t, dark, hueDeg, o) => {
  const cx = size / 2;
  const cy = size / 2;
  const R = (size / 2) * 0.78;
