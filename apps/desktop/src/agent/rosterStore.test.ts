...
describe("an agent's policies", () => {
  // Policies are the mirror of capabilities and follow the same overlay rule:
  // null on an unedited row inherits the preset's prohibitions, and the shipped
  // built-ins carry none.
  test("an unedited row inherits its policies (null)", () => {
    expect(seeded().listAgents()[0]!.policies).toBeNull();
  });

  test("a new agent keeps the policies it was made with", () => {
    const made = seeded().createAgent({
      name: "Ama",
      policies: { deniedCommands: ["rm -rf"], deniedPaths: [".env"] },
    })!;
    expect(made.policies).toEqual({ deniedCommands: ["rm -rf"], deniedPaths: [".env"] });
  });

  test("a new agent left silent about its policies inherits them", () => {
    expect(seeded().createAgent({ name: "Ama" })!.policies).toBeNull();
  });

  // The whole object is the overlay unit: an object with empty lists is a real
  // "forbids nothing" answer stored as itself, while null hands the field back.
  test("an empty-lists object is stored as an answer; null hands the field back", () => {
    const store = seeded();
    store.updateAgent("kone", { policies: { deniedCommands: ["rm"], deniedPaths: [] } });
    expect(
      store.updateAgent("kone", { policies: { deniedCommands: [], deniedPaths: [] } })?.policies,
    ).toEqual({ deniedCommands: [], deniedPaths: [] });
    expect(store.updateAgent("kone", { policies: null })?.policies).toBeNull();
  });

  test("policies left out of a patch are left alone", () => {
    const store = seeded();
    store.updateAgent("kone", { policies: { deniedCommands: ["rm -rf"], deniedPaths: [".env"] } });
    const after = store.updateAgent("kone", { model: { provider: "codex", model: "gpt-5" } })!;
    expect(after.policies).toEqual({ deniedCommands: ["rm -rf"], deniedPaths: [".env"] });
  });

  // A blank command or path is not a prohibition: an empty entry is dropped
  // rather than stored as a rule that would match everything or nothing.
  test("a blank entry is dropped, not stored", () => {
    const made = seeded().createAgent({
      name: "Ama",
      policies: { deniedCommands: ["", "rm -rf"], deniedPaths: ["  ", ".env"] },
    })!;
  ...
  test("a list longer than the row allows is clamped, not refused", () => {
    const many = Array.from({ length: 200 }, (_, i) => `cmd-${i}`);
    const made = seeded().createAgent({
      name: "Ama",
      policies: { deniedCommands: many, deniedPaths: [] },
    })!;
    expect(made.policies!.deniedCommands).toHaveLength(128);
  });

  test("a fork carries policies, the row's own winning over the preset's", () => {
    const store = seeded();
    store.updateAgent("kone", { policies: { deniedCommands: ["rm -rf"], deniedPaths: [] } });
    const copy = store.duplicateAgent({
      agentId: "kone",
      newAgentId: "copy-1",
      inherited: {
        name: "kone",
        policies: { deniedCommands: ["curl"], deniedPaths: ["secrets"] },
      },
    })!;
    expect(copy.policies).toEqual({ deniedCommands: ["rm -rf"], deniedPaths: [] });
  });

  test("policies survive a round trip through the database", () => {
    const store = seeded();
    store.updateAgent("kone", {
      policies: { deniedCommands: ["rm -rf", "git push"], deniedPaths: [".env", "secrets"] },
    });
    store.close();

    const reopened = new ConversationStoreCtor();
    expect(reopened.getAgent("kone")!.policies).toEqual({
