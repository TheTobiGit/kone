...
/** A dictionary whose key is a bare `string`/`number`/`symbol` — or an index
 *  signature, or a mapped type over one — is not a widening target. The
 *  annotation is what permits the dynamic index in the first place, so there is
 *  no version of the table that both keeps its literal value types and still
 *  answers `table[runtimeKey]`: dropping it for `satisfies` infers only the
 *  literal keys present and every lookup stops compiling with TS7053. Measured,
 *  not assumed — 75 of 90 attempted conversions failed exactly that way. The
 *  evidence the rule wants preserved is unreachable anyway, since the consumer's
 *  key is not known at the call site, and with `noUncheckedIndexedAccess` on the
 *  lookup already yields `V | undefined` — the safety the evidence would have
 *  bought. A key that closes over a union of literals (`Record<Cue, V>`) is a
 *  different matter: those convert cleanly, 45 of them did, and they stay
 *  reported. */
export function classifyWideningTarget(
	...
	if (unwrapped.type === "TSTypeLiteral") {
		// Open key, so out of scope — see the note above `classifyWideningTarget`.
		if (unwrapped.members.some((member) => member.type === "TSIndexSignature")) return null;
...
function classifyAliasBroadTarget(
	...
	if (unwrapped.type === "TSUnknownKeyword") return { kind: "unknown" };
	if (unwrapped.type === "TSObjectKeyword") return { kind: "object" };
	// The same open-key exclusion, where the dictionary is reached through an
	// alias: an index signature has no closed key set to preserve.
