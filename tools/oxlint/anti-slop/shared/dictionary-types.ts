...
function openDictionaryKey(
	...
	if (substitution !== undefined) {
		return (
			...
			openDictionaryKey(substitution, environment, substitutions, resolvingAliases)
		);
	}
	if (TRANSPARENT_WRAPPERS.has(name) && isBuiltIn(name, environment)) {
		const wrapped = unwrapped.typeArguments?.params[0];
		return (
			wrapped !== undefined &&
			openDictionaryKey(wrapped, environment, substitutions, resolvingAliases)
		);
	}
	if (name === "Record" && isBuiltIn(name, environment)) {
	...
	const nextResolving = new Set(resolvingAliases);
	nextResolving.add(name);
	return openDictionaryKey(alias.typeAnnotation, environment, nextSubstitutions, nextResolving);
}

/** Whether a dictionary type admits any key of a primitive kind — `Record<string,
 *  V>`, an index signature, `{ [K in PropertyKey]: V }` — rather than closing over
 *  a known union of literals like `Record<Cue, V>`. The distinction matters to a
 *  caller that wants to know whether the key set is enumerable. */
export function hasOpenDictionaryKey(
	type: ESTree.TSType,
	environment: TypeEnvironment,
): boolean {
	return openDictionaryKey(type, environment, new Map(), new Set());
}
