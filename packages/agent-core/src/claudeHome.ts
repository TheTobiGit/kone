...
export function resolveClaudeExecutable(): string | undefined {
  // SAFETY: the assertion only widens `process` with Electron's
  // `resourcesPath` property. The runtime check below treats its absence as
  // "not packaged": under plain Node it is undefined and the caller omits the
  // executable option (fine unpacked).
